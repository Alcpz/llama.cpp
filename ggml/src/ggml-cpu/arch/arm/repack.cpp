    ...
    for (int y = 0; y < nr / q8_k_rows; y++) {
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            for (int b = 0; b < nb; b++) {
                ...
                float32x4_t sbd_scale_0123[q8_k_rows];
                float32x4_t sbd_scale_4567[q8_k_rows];
                float32x4_t sbd_min_0123[q8_k_rows];
                float32x4_t sbd_min_4567[q8_k_rows];
                ...
                const int16x8_t bsums[q8_k_rows] = {
                ...
                for (int sb = 0; sb < QK_K / 64; sb++) {
                    ...
                    const int16x4_t sc_0123_hi = vget_low_s16(q4sb_scales[1]);
                    const int16x4_t sc_4567_hi = vget_high_s16(q4sb_scales[1]);
                    for (int row = 0; row < q8_k_rows; row++) {
                        // Bias correction
                        // row c0123 blk0 and blk1
            ...
            for (int i = 0; i < q8_k_rows; i++) {
                int row = y * q8_k_rows + i;
                for (int j = 0; j < 2; j++) {
...
void ggml_gemm_q4_K_8x8_q8_K(int                        n,
    ...
    constexpr int    q8_k_rows_interleaved = 4;
    const uint8x16_t m4b           = vdupq_n_u8(0x0f);

    // 8 accumulators: 2 row pairs × 4 col pairs
    float32x4_t acc_f32[blocklen];

    for (int y = 0; y < nr / q8_k_rows_interleaved; y++) {
        const block_q8_Kx4 * GGML_RESTRICT q8_ptr = (const block_q8_Kx4 *) vy + (y * nb);
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            for (int b = 0; b < nb; b++) {
                ...
                for (int i = 0; i < q8_k_rows_interleaved; i++) {
                    for (int j = 0; j < 2; j++) {
            ...
            // With the previous reorder, the tile is already in the correct memory layout.
            for (int i = 0; i < q8_k_rows_interleaved; i++) {
                int row = y * q8_k_rows_interleaved + i;
                for (int j = 0; j < 2; j++) {
                    int col    = x * ncols_interleaved + j * 4;
                    int offset = row * bs + col;
                    vst1q_f32(s + offset, acc_f32[2 * i + j]);
