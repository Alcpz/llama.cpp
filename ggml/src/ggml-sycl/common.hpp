...
[[noreturn]] static void ggml_sycl_error(const char * stmt, const char * func, const char * file, const int line,
                                         const char * msg) {
...
#define SYCL_CHECK(err)                                                                           \
    do {                                                                                          \
        auto err_ = (err);                                                                        \
        if (err_ != 0)                                                                            \
            ggml_sycl_error(#err, __func__, __FILE__, __LINE__, "Meet error in this line code!"); \
    } while (0)
...
template <typename T> struct ggml_sycl_pool_alloc {
    /* device */ ggml_sycl_pool * pool        = nullptr;
    T *                           ptr         = nullptr;
...
template <float (*bin_op)(const float, const float)> struct bin_bcast_sycl {
    ...
    void operator()(ggml_backend_sycl_context & ctx, const struct ggml_tensor * src0, const struct ggml_tensor * src1,
                    struct ggml_tensor * dst, const src0_t * src0_dd, const src1_t * src1_dd, dst_t * dst_dd,
                    queue_ptr stream) {
...
inline void ggml_sycl_op_bin_bcast(ggml_backend_sycl_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1,
    ...
    if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        op()(ctx, src0, src1, dst, (const sycl::half *) src0_dd, src1_dd, (sycl::half *) dst_dd, main_stream);
    } else if (src0->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F32) {
        op()(ctx, src0, src1, dst, (const sycl::half *) src0_dd, src1_dd, dst_dd, main_stream);
...
void ggml_sycl_op_flatten(ggml_backend_sycl_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1,
                          ggml_tensor * dst, const ggml_sycl_op_flatten_t op);
